...
| Token di risposta | Supportato | Supportato. Consulta la [documentazione sui token di risposta dedicati](https://experienceleague.adobe.com/docs/target/using/administer/response-tokens.html?lang=it) per esempi di codice e differenze tra at.js e Platform Web SDK |
...
| `applyOffer()` e `applyOffers()` | Utilizzare il comando [`applyPropositions`](https://experienceleague.adobe.com/docs/experience-platform/edge/personalization/rendering-personalization-content.html?lang=it#applypropositions) per applicare il contenuto. Puoi scegliere di impostare, sostituire o aggiungere HTML a un selettore specifico. |
| `triggerView()` | Platform Web SDK attiva automaticamente una [modifica della visualizzazione](https://experienceleague.adobe.com/docs/experience-platform/edge/personalization/adobe-target/web-sdk-atjs-comparison.html?lang=it#how-to-trigger-a-view-change-in-a-single-page-application) ai fini del Compositore esperienza visivo per SPA se la proprietà `web.webPageDetails.viewName` è impostata nell&#39;opzione `xdm` del comando `sendEvent`. |
