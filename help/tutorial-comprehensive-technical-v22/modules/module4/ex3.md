---
title: Servizio query - Query, query, query... e analisi di abbandono
description: Servizio query - Query, query, query... e analisi di abbandono
kt: 5342
...
# 4.3 Query, query, query... e analisi di abbandono

## Oggetto

* Query di scrittura per analisi dati
* Scrivi query SQL combinando dati online, call center e fedeltà disponibili in Adobe Experience Platform
* Scopri le funzioni definite da Adobe

## Contesto

In questi esercizi scriverai query per analizzare le visualizzazioni di prodotto, i funnel di prodotto, la coorte, ecc.

Tutte le query elencate in questo capitolo verranno eseguite nel **Interfaccia della riga di comando PSQL**. È necessario copiare (CTRL+C) i blocchi di istruzione indicati con **SQL** e li incolla (CTRL+V) nel **Interfaccia della riga di comando PSQL**. La **Risultato query** i blocchi mostrano l&#39;istruzione SQL incollata e il risultato della query associato.

## 4.3.1 Scrivere query di base per l&#39;analisi dei dati

### Marca temporale

I dati acquisiti in Adobe Experience Platform sono contrassegnati con marca temporale. La **timestamp** attributo ti consente di analizzare i dati nel tempo.
...
Copia l’istruzione di cui sopra ed eseguiscila nel **Interfaccia della riga di comando PSQL**.
...
### Identifica i visitatori con rischio di abbandono (visita pagina => Annulla servizio)
...
Nel successivo set di query estenderemo la query di cui sopra, al fine di ottenere una visione completa dei clienti e del loro comportamento che hanno visitato la pagina &quot;Annulla servizio&quot;. Scoprirai come utilizzare la funzione definita dall’Adobe per sessionizzare le informazioni, identificare la sequenza e la tempistica degli eventi. Inoltre, unirai i set di dati per arricchire e preparare ulteriormente i dati per l’analisi in Microsoft Power BI.

## 4.3.2 Query avanzate

La maggior parte della logica di business richiede la raccolta dei punti di contatto per un cliente e l’ordine per tempo. Questo supporto viene fornito da SQL Spark sotto forma di funzioni finestra. Le funzioni finestra fanno parte di SQL standard e sono supportate da molti altri motori SQL.

### Funzioni definite da Adobe

Adobe ha aggiunto un set di **Funzioni definite dall&#39;Adobe** alla sintassi SQL standard che consente di comprendere meglio i dati dell&#39;esperienza. Nel paio di query successivo si imparerà su queste funzioni ADF. Ulteriori informazioni e l&#39;elenco completo [nella documentazione](https://experienceleague.adobe.com/docs/experience-platform/query/sql/adobe-defined-functions.html).

### Cosa fanno le persone sul sito prima di raggiungere la pagina &quot;Annulla servizio&quot; come terza pagina in una sessione?

Con questa query scoprirete le prime due funzioni definite dall&#39;Adobe **SESS_TIMEOUT** e **SUCCESSIVO**

> La **SESS_TIMEOUT()** riproduce i raggruppamenti di visite trovati con Adobe Analytics. Esegue un raggruppamento temporale simile, ma con parametri personalizzabili.
>
> **NEXT()** e **PREVIOUS()** aiuta a capire come i clienti navigano nel tuo sito.
...
Per rispondere a questo tipo di query utilizzeremo il **TIME_BETWEEN_NEXT_MATCH()** Funzione definita dall&#39;Adobe.
...
### E qual è il risultato di quel contatto?

Spiega che stiamo unendo i set di dati, in questo caso ci uniamo al nostro `demo_system_event_dataset_for_website_global_v1_1` con `demo_system_event_dataset_for_call_center_global_v1_1`. Lo facciamo per conoscere il risultato dell&#39;interazione con il call center.
...
In questa query uniamo i dati fedeltà caricati in Adobe Experience Platform. Questo consente di arricchire l’analisi di abbandono con i dati fedeltà.
...
Includiamo le informazioni geografiche, come longitudine, atteggiamento, città, codice della campagna, acquisite dal Adobe Experience Platform per ottenere alcune informazioni geografiche sui clienti in continua crescita.
...
## Analisi dell&#39;interazione con il call center

Nelle query di cui sopra abbiamo guardato solo i visitatori che hanno finito per contattare il call center in caso di cancellazione del servizio. Vogliamo prendere in considerazione questo aspetto un po&#39; più ampio e tenere conto di tutte le interazioni con i call center (wifi, promo, fattura, reclamo e contratto).

Sarà necessario modificare una query, quindi apriamo prima il blocco note o le parentesi.

In Windows fare clic su &quot;search&quot;-icon (1) nella barra degli strumenti di Windows, digitare **blocco note** nel campo &quot;search&quot; (2), fare clic (3) sul risultato &quot;blocco note&quot;:
...
Copiare la seguente istruzione su blocco note/parentesi:
...
Non rimuovere `/\*` e `\*/`. L&#39;istruzione modificata nel blocco note dovrebbe essere simile a:

![edit-query-blocco note.png](./images/edit-query-notepad.png)

Copia l&#39;istruzione modificata da **blocco note** nel **Finestra della riga di comando PSQL** e premi invio. Nella finestra della riga di comando PSQL dovrebbe essere visualizzato il seguente risultato:
...
Nel successivo persisterai la query (nota anche come **crea tabella come selezione** o **CTAS**) come nuovo set di dati da utilizzare in Microsoft Power BI.

Passaggio successivo: [4.4 Generare un set di dati da una query](./ex4.md)
